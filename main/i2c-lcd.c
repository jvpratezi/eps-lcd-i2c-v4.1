...
void lcd_send_cmd (char cmd)
{
  char data_u, data_l;
    ...
    i2c_cmd_handle_t cdm = i2c_cmd_link_create();
    i2c_master_start(cdm);
    i2c_master_write_byte(cdm, (SLAVE_ADDRESS_LCD << 1) | WRITE_BIT, ACK_CHECK_EN);
    i2c_master_write(cdm, data_t, 4, ACK_CHECK_EN);
    i2c_master_stop(cdm);
    esp_err_t ret = i2c_master_cmd_begin(I2C_NUM, cdm, 1000 / portTICK_RATE_MS);
